...
# Main
# ----------------------------
@app.route('/api/get_all_detections')
def get_all_detections():
    """
    Sends all detection data (lat, lng, score) as JSON.
    """
    try:
        cursor = detections_collection.find(
            {"latitude": {"$ne": None}, "longitude": {"$ne": None}},
            {"latitude": 1, "longitude": 1, "pollution_score": 1, "location_name": 1, "timestamp": 1}
        ).sort("timestamp", -1)
        ...
        detections = [
            {
                ...
                "name": d.get("location_name", "Unknown"),
        ...
        print(f"Error in /api/get_all_detections: {e}")
